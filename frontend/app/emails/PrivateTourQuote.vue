    ...
    <Body style="background-color: #f5f5f5; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif;">
      <Container style="background-color: #ffffff; margin: 40px auto; padding: 0; max-width: 600px; border-radius: 8px; overflow: hidden; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1);">
        ...
        <Section style="padding: 40px 30px;">
          <Heading as="h2" style="color: #1a2232; font-size: 24px; font-weight: 700; margin: 0 0 20px 0;" th:text="#{email.private.quote.title}">
            Tu Cotización de Tour Privado
          </Heading>

          <Text style="margin: 16px 0; color: #374151;" th:text="#{email.private.quote.greeting(${customerName})}">
            Hola <span>{{ props.customerName }}</span>,
          </Text>

          <Text style="margin: 16px 0; color: #374151;" th:text="#{email.private.quote.intro}">
          ...
          <Section style="background-color: #f8fafc; border-left: 4px solid #fbaa62; border-radius: 8px; padding: 20px 24px; margin: 24px 0;">
            <Text style="margin: 0 0 16px 0; color: #1a2232; font-size: 18px; font-weight: 700;" th:text="#{email.private.quote.details}">
              Detalles del Tour
            </Text>

            <Text style="margin: 0 0 12px 0; color: #1a2232;">
              <strong th:text="#{email.private.quote.type}">Tipo de Tour:</strong><br>
              <span th:text="${tourType}" style="color: #64748b;">{{ props.tourType }}</span>
            </Text>
            ...
            <Text style="margin: 20px 0 0 0; color: #1a2232; font-size: 24px; font-weight: 700;">
              ...
              <span style="color: #fbaa62;" th:text="${price}">{{ props.price }}</span>
            </Text>
          </Section>

          <!-- Validity Notice -->
          <Section style="background-color: #dbeafe; border-left: 4px solid #3b82f6; border-radius: 6px; padding: 16px; margin: 24px 0;">
            <Text style="margin: 0; color: #1e40af; font-size: 14px;" th:text="#{email.private.quote.validity}">
          ...
          <div th:if="${requests != null and !requests.isEmpty()}">
            <Text style="margin: 24px 0 8px 0; color: #1a2232; font-weight: 700;" th:text="#{email.private.quote.requests}">
            ...
            <Section style="background-color: #f0fdf4; border-left: 4px solid #10b981; border-radius: 6px; padding: 16px; margin: 8px 0 24px 0;">
              <Text style="margin: 0; color: #065f46; white-space: pre-wrap;" th:text="${requests}">
                {{ props.requests }}
              </Text>
            </Section>
          </div>
          ...
          <div th:if="${notes != null and !notes.isEmpty()}">
            <Text style="margin: 24px 0 8px 0; color: #1a2232; font-weight: 700;" th:text="#{email.private.quote.notes}">
              Notas:
            </Text>
            <Text style="margin: 0 0 24px 0; padding: 16px; background-color: #fef3c7; border-radius: 6px; color: #92400e; border: 1px solid #fcd34d; white-space: pre-wrap;" th:text="${notes}">
              {{ props.notes }}
            </Text>
          </div>
          ...
          <Section style="background-color: #f8fafc; border-radius: 8px; padding: 20px; margin: 32px 0;">
            <Text style="margin: 0; color: #374151; line-height: 1.6;" th:text="#{email.private.quote.next}">
          ...
          <Section style="text-align: center; margin: 32px 0;">
            <a
              href="mailto:contacto@northernchile.cl"
