<template>
  <div class="min-h-screen bg-neutral-50 dark:bg-neutral-900 py-12">
    <UContainer>
      ...
      <UCard class="mb-8 bg-tertiary/5 border-tertiary/20">
        <div class="flex items-start gap-4">
          ...
          <div class="flex-1">
            ...
            <div v-if="nextFullMoons && nextFullMoons.length > 0" class="space-y-2">
              <div
      ...
      <!-- Info Section -->
      <div class="mt-8 bg-gradient-to-br from-tertiary/5 to-primary/5 rounded-lg p-6 border border-tertiary/20">
        <div class="flex items-start gap-4">
          <UIcon name="i-lucide-info" class="w-6 h-6 text-tertiary flex-shrink-0 mt-1" />
          <div>
            <h3 class="text-lg font-semibold text-neutral-900 dark:text-white mb-2">
...
const fetchMoonData = async () => {
  ...
  try {
    ...
    const response = await $fetch(`${config.public.apiBaseUrl}/lunar/calendar`, {
...
const fetchNextFullMoons = async () => {
  try {
    const response = await $fetch(`${config.public.apiBaseUrl}/lunar/next-full-moons`, {
      params: { count: 3 }
    })
