<template>
  <div class="min-h-screen bg-neutral-50 dark:bg-neutral-900 py-6">
    <UContainer>
      ...
      <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        ...
        <UCard>
          <div class="flex items-center gap-3">
            ...
            <div>
              <p class="text-sm text-neutral-600 dark:text-neutral-400">Creados</p>
              <p class="text-2xl font-bold text-neutral-900 dark:text-white">
                {{ stats.createActions }}
              </p>
            </div>
        ...
        <UCard>
          <div class="flex items-center gap-3">
            ...
            <div>
              <p class="text-sm text-neutral-600 dark:text-neutral-400">Actualizados</p>
              <p class="text-2xl font-bold text-neutral-900 dark:text-white">
                {{ stats.updateActions }}
              </p>
            </div>
        ...
        <UCard>
          <div class="flex items-center gap-3">
            ...
            <div>
              <p class="text-sm text-neutral-600 dark:text-neutral-400">Eliminados</p>
              <p class="text-2xl font-bold text-neutral-900 dark:text-white">
                {{ stats.deleteActions }}
              </p>
            </div>
      ...
      <div v-else-if="!auditLogs || auditLogs.length === 0" class="text-center py-12 bg-white dark:bg-neutral-800 rounded-lg">
        <UIcon name="i-lucide-file-search" class="w-16 h-16 mx-auto text-neutral-400 mb-4" />
      ...
      <div v-else>
          ...
          >
            <div class="flex gap-4">
              ...
              <div class="flex-1 min-w-0">
                <div class="flex items-start justify-between gap-4 mb-2">
                  <div class="flex-1">
                    ...
                    <h3 class="text-lg font-semibold text-neutral-900 dark:text-white">
                      {{ log.entityDescription || `${log.entityType} ${log.entityId}` }}
                    </h3>
                  </div>
                ...
                <div class="grid md:grid-cols-2 gap-2 text-sm text-neutral-600 dark:text-neutral-400 mb-3">
                ...
                <!-- Old/New Values -->
                <div v-if="log.oldValues || log.newValues" class="mt-3 pt-3 border-t border-neutral-200 dark:border-neutral-700">
                  <details class="group">
                    <summary class="cursor-pointer text-sm font-medium text-neutral-700 dark:text-neutral-300 hover:text-primary transition-colors">
                      Ver detalles
                      <UIcon
                    ...
                    <div class="mt-3 space-y-3">
                      <div v-if="log.oldValues && Object.keys(log.oldValues).length > 0">
                        <p class="text-xs font-medium text-neutral-600 dark:text-neutral-400 mb-1">
                          Valores Anteriores:
                        </p>
                        <pre class="text-xs bg-neutral-100 dark:bg-neutral-900 p-3 rounded overflow-x-auto">{{ JSON.stringify(log.oldValues, null, 2) }}</pre>
                      </div>

                      <div v-if="log.newValues && Object.keys(log.newValues).length > 0">
                        <p class="text-xs font-medium text-neutral-600 dark:text-neutral-400 mb-1">
                          Valores Nuevos:
                        </p>
                        <pre class="text-xs bg-neutral-100 dark:bg-neutral-900 p-3 rounded overflow-x-auto">{{ JSON.stringify(log.newValues, null, 2) }}</pre>
                      </div>
                    </div>
                  </details>
                </div>
        ...
        <!-- Pagination -->
        <div class="flex items-center justify-between mt-6 bg-white dark:bg-neutral-800 rounded-lg p-4">
...
const fetchAuditLogs = async () => {
  ...
  try {
    ...
    const response = await $fetch(`${config.public.apiBaseUrl}/admin/audit-logs?${params.toString()}`, {
      credentials: 'include'
    })
...
const fetchStats = async () => {
  try {
    const response = await $fetch(`${config.public.apiBaseUrl}/admin/audit-logs/stats`, {
      credentials: 'include'
    })
